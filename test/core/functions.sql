...
/*
 * _pg_classy.name_sanity()
 */
CREATE FUNCTION run__name_sanity(
  text
) RETURNS text LANGUAGE sql AS $body$
  SELECT $$SELECT _pg_classy.name_sanity( 'field_name', $$
    || quote_nullable($1) || ' )'
$body$;
...
() RETURNS SETOF text LANGUAGE plpgsql AS $body$
DECLARE
  c_schema CONSTANT name := '_pg_classy';
...
/*
 * FUNCTION _pg_classy.language__get_id
 */
...
DECLARE
  ...
  template CONSTANT text := $$SELECT _pg_classy.language__get_id( %L )$$;
BEGIN
  RETURN NEXT is(
    _pg_classy.language__get_id( get_test_language_name() )
    , v_id
...
() RETURNS SETOF text LANGUAGE plpgsql AS $body$
DECLARE
  c_schema CONSTANT name := 'pg_classy';
...
BEGIN
  ...
  RETURN NEXT bag_eq(
    ...
    , format( $$SELECT %s FROM _pg_classy.language$$, v_columns )
    , $$template_language returns same results as _pg_classy.language$$
  );
  EXCEPTION
...
CREATE FUNCTION run_template_language__add(
  ...
  SELECT 'SELECT pg_classy.template_language__add( '
    || quote_nullable($1) || ', '
    || quote_nullable($2) || ', '
    || quote_nullable($3) || ', '
    || quote_nullable($4) || ', '
...
BEGIN
  ...
  RETURN NEXT ok(
    EXISTS(SELECT 1 FROM variant.allowed_types( 'pg_classy_template' ) WHERE allowed_type = 'text'::regtype)
    , 'Verify type added to registered variant "pg_classy_template"'
  );
  RETURN NEXT ok(
    EXISTS(SELECT 1 FROM variant.allowed_types( 'pg_classy_parameter' ) WHERE allowed_type = 'text[]'::regtype)
    , 'Verify type added to registered variant "pg_classy_parameter"'
  );
  RETURN NEXT ok(
    EXISTS(SELECT 1 FROM variant.allowed_types( 'pg_classy_return' ) WHERE allowed_type = 'text[]'::regtype)
    , 'Verify type added to registered variant "pg_classy_return"'
  );

  RETURN NEXT function_privs_are(
    'pg_classy', 'template_language__add'
...
/*
 * FUNCTION pg_classy.template__add
 */
 ...
 */
CREATE FUNCTION any_to_pg_classy_template(
  anyelement
) RETURNS variant.variant(pg_classy_template) LANGUAGE plpgsql VOLATILE AS $$BEGIN RETURN $1::variant.variant(pg_classy_template); END$$;
CREATE FUNCTION any_to_pg_classy_parameter(
  anyelement
) RETURNS variant.variant(pg_classy_parameter) LANGUAGE plpgsql VOLATILE AS $$BEGIN RETURN $1::variant.variant(pg_classy_parameter); END$$;
...
BEGIN
  ...
  EXCEPTION
    WHEN undefined_table THEN
      ids[1] := pg_classy.template__add( get_test_language_name(), 'test template', any_to_pg_classy_template('test 1'::text) );
      ids[2] := pg_classy.template__add( get_test_language_name(), 'test template', 2, any_to_pg_classy_template('test 2'::text) );
...
--  RAISE WARNING 'v_array_allowed %', v_array_allowed;
  IF NOT v_array_allowed THEN
    PERFORM variant.add_type( 'pg_classy_template', 'text[]' );
  END IF;

  RETURN NEXT throws_ok(
    $$SELECT pg_classy.template__add( bogus_language_name(), 'test_template', any_to_pg_classy_template('test 1'::text) )$$
    , format( 'language "%s" not found', bogus_language_name() )
  ...
  RETURN NEXT throws_ok(
    $$SELECT pg_classy.template__add( get_test_language_name(), 'test_template', 9, '{a,b}'::text[]::variant.variant(pg_classy_template) )$$
    , '12345'
    , NULL
  ...
  IF NOT v_array_allowed THEN
    PERFORM variant.remove_type( 'pg_classy_template', 'text[]' );
  END IF;
   */
  ...
  RETURN NEXT bag_eq(
    ...
    $$SELECT language_name
          ...
          , template_version
          , template::text
        FROM _pg_classy.template t
          JOIN _pg_classy.language l USING( language_id )
...
/*
 * FUNCTION _pg_classy.template__get
 */
...
BEGIN
  ...
  RETURN NEXT throws_ok(
    format( $$SELECT _pg_classy.template__get( 'bogus' )$$ )
    , 'P0002'
    , format( $$template name "bogus" at version 1 not found$$, get_test_language_name() )
  );

  RETURN NEXT is(
    ( SELECT row(g.*)::_pg_classy.template FROM _pg_classy.template__get( 'bogus', loose := true ) AS g )
    , NULL::_pg_classy.template
    , 'Verify loose := true'
  );

  RETURN NEXT bag_eq(
    $$SELECT * FROM _pg_classy.template__get( 'test template' )$$
    , $$SELECT *
          FROM _pg_classy.template
          WHERE language_id = get_test_language_id()
            AND template_name = 'test template'
            AND template_version = 1
      $$
    , $$Check _pg_classy.template__get( ..., 'test template' )$$
  );
  /*
  RETURN NEXT bag_eq(
    $$SELECT * FROM _pg_classy.template__get( get_test_language_name(), 'test template', i ) FROM generate_series(1, 2) i$$
    , $$SELECT * FROM verify_test_template$$
    , $$Check _pg_classy.template__get( ..., 'test template', version )$$
  );
  */
  RETURN QUERY SELECT is(
        _pg_classy.template__get( template_name, template_version )
        , ROW(t.*)::_pg_classy.template
        , format( $$Check _pg_classy.template__get( 'test template', %s )$$, template_version )
      )
    FROM _pg_classy.template t
      LEFT JOIN _pg_classy.language l USING( language_id )
  ;
...
/*
 * FUNCTION pg_classy.template__remove
 */
...
DECLARE
  ...
  test_sql CONSTANT text := 
    $$SELECT * FROM _pg_classy.template WHERE template_id = ANY( %L )$$
  ;
BEGIN
  ids := get_test_templates();
  PERFORM pg_classy.template__remove( 'test template' );
  PERFORM pg_classy.template__remove( 'test template', 2 );
  ...
  ids := get_test_templates();
  PERFORM pg_classy.template__remove( id ) FROM unnest(ids) AS i(id);
  DROP TABLE test_templates;
...
/*
 * FUNCTION pg_classy.template__dependency
 */
...
BEGIN
  ...
  test_table := '_pg_classy_test.test_dependency';
  test_field := 'test_template_id';
  DROP TABLE IF EXISTS _pg_classy_test.test_dependency;
  CREATE TABLE _pg_classy_test.test_dependency(
  ...
  DECLARE
    ...
    errcontext text := $$\APL/pgSQL function pg_classy.template__dependency__add\(text,name\) line \d+ during statement block local variable initialization$$;

    context text;
    description text := 'threw with proper context ' || errcode || ': ' || errmsg;
  BEGIN
    PERFORM pg_classy.template__dependency__add( 'bogus_table', test_field );
  ...
  RETURN NEXT throws_ok(
    $$SELECT pg_classy.template__dependency__add( 'test_dependency', 'bogus_field' )$$
    , '42703'
    , NULL
  ...
  DECLARE
    test_template CONSTANT text :=
        $test$SELECT pg_temp.exec_as( '_pg_classy_test_role', $sql$SELECT pg_classy.template__dependency__%s( %L, %L )$sql$ )$test$
    ;
  BEGIN
    DROP ROLE IF EXISTS _pg_classy_test_role;
    CREATE ROLE _pg_classy_test_role;
    GRANT USAGE ON SCHEMA _pg_classy_test TO _pg_classy_test_role;
    GRANT USAGE ON SCHEMA _pg_classy TO _pg_classy_test_role;
    ALTER TABLE _pg_classy_test.test_dependency OWNER TO _pg_classy_test_role;
    RETURN NEXT throws_ok(
    ...
    GRANT pg_classy__dependency TO _pg_classy_test_role;
    ...
    RETURN NEXT fk_ok(
      '_pg_classy_test', 'test_dependency', test_field
      , '_pg_classy', 'template', 'template_id'
    );
    ...
    RETURN NEXT col_isnt_fk( '_pg_classy_test', 'test_dependency', test_field, $$FK does not exist$$ );
    ...
    DROP TABLE _pg_classy_test.test_dependency;
    REVOKE USAGE ON SCHEMA _pg_classy_test FROM _pg_classy_test_role;
    REVOKE USAGE ON SCHEMA _pg_classy FROM _pg_classy_test_role;
    DROP ROLE IF EXISTS _pg_classy_test_role;
  END;
END
$body$;
...
() RETURNS SETOF text LANGUAGE plpgsql AS $body$
DECLARE
  c_test_role CONSTANT name := 'Completely bogus pg_classy test role';
BEGIN
  RETURN NEXT lives_ok(
  ...
  RETURN NEXT lives_ok(
    format( 'GRANT USAGE ON SCHEMA _pg_classy_test TO %I', c_test_role )
...
BEGIN
  ...
  RETURN NEXT lives_ok(
    format( 'REVOKE USAGE ON SCHEMA _pg_classy_test FROM %I', c_test_role )
...
/*
 * FUNCTION pg_classy.process
 */
CREATE FUNCTION test_process
() RETURNS SETOF text LANGUAGE plpgsql AS $body$
DECLARE
  lname CONSTANT text := get_test_language_name();
  test_l CONSTANT text := $$SELECT pg_classy.process_language( %L, %L, %L )$$;
  test CONSTANT text := replace( test_l, '_language', '' );
  template_name CONSTANT text := 'test template';
  p CONSTANT text := 'pg_classy.process(): ';
  c_original_role CONSTANT name := current_user;
BEGIN
  PERFORM get_test_language_id();
  PERFORM variant.add_type( 'pg_classy_template', 'varchar' );

  RETURN NEXT throws_ok(
    format( test_l, bogus_language_name(), any_to_pg_classy_template('%s'::text), any_to_pg_classy_parameter('{a}'::text[]) )
    , 'P0002'
  ...
  RETURN NEXT throws_ok(
    format( test_l, lname, any_to_pg_classy_template('%s'::varchar), any_to_pg_classy_parameter('{a}'::text[]) )
    , '22000'
  ...
  RETURN NEXT throws_ok(
    format( test_l, lname, any_to_pg_classy_template('%s'::text), any_to_pg_classy_parameter('{a}'::text) )
    , '22000'
  ...
  RETURN QUERY SELECT row(oid::regprocedure, prorettype::regtype, prosrc)::text
    FROM pg_proc
    WHERE pronamespace = (SELECT oid FROM pg_namespace WHERE nspname='_pg_classy_functions')
  ;
  */
  ...
  RETURN NEXT lives_ok($lives$
    ...
    SELECT *
          ...
          , any_to_pg_classy_template(template) AS template_v
          , any_to_pg_classy_parameter(parameters) AS parameters_v
      FROM (
          SELECT *
            FROM (VALUES
  ...
  RETURN NEXT lives_ok(
    format(
        $$SELECT pg_classy.template__add( %L, %L, version, template ) FROM test$$
        , lname
        , template_name
      )
  ...
  RETURN QUERY
    SELECT is(
           ...
           */
          pg_classy.process_language( lname, template_v, parameters_v )
          , expected
          , format( 'pg_classy.process( ..., %L, %L )', template, parameters )
        )
      FROM test
  ;
  ...
  BEGIN
    RETURN QUERY
      SELECT is(
            pg_classy.process( template_name, parameters_v )
            , expected
            , format( 'pg_classy.process( %L, %L )', template_name, parameters )
          )
        FROM test
        WHERE version = 1
    ;
    ...
    RETURN QUERY
      SELECT is(
            pg_classy.process( template_name, version, parameters_v )
            , expected
            , format( 'pg_classy.process( %L, %L, %L )', template_name, version, parameters )
          )
        FROM test
    ;
  EXCEPTION
  ...
  RETURN QUERY SELECT _pg_classy_test.drop_test_role(c_original_role);
END
$body$;

/*
 * FUNCTION pg_classy.extract_parameters
 */
...
BEGIN
  ...
  BEGIN
    RETURN NEXT is(
      pg_classy.extract_parameters( lname, any_to_pg_classy_parameter('{cow,goes,moo}'::text[]), '{2}' )::text[]
      , array['goes'::text]
    );
  EXCEPTION
    WHEN others THEN
...
/*
 * FUNCTION pg_classy.execute
CREATE FUNCTION test_execute
() RETURNS SETOF text LANGUAGE plpgsql AS $body$
DECLARE
  lname CONSTANT text := get_test_language_name();
  test CONSTANT text := $$SELECT pg_classy.execute( %L, %L, %L )$$;
  template_name CONSTANT text := 'test template';
BEGIN
  PERFORM get_test_language_id();
  PERFORM variant.add_type( 'pg_classy_template', 'varchar' );
  PERFORM variant.add_type( 'pg_classy_parameter', 'text' );
 */
